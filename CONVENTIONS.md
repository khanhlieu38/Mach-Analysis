...
**Hierarchy:** `CONVENTIONS.md` (repo) → `studies/<study>/STUDY_RULES.md` (sample, schema, taxonomy study) → prompt (task từng phiên). Nếu mâu thuẫn, **STUDY_RULES thắng** cho study đó.

Entry points AI: [AGENTS.md](AGENTS.md) (Cursor) · [CLAUDE.md](CLAUDE.md) (Claude Code) — cả hai trỏ tới file này + STUDY_RULES.
...
- `studies/<study>/findings.qmd` — phân tích đầy đủ: patterns + hypothesis + signals (taxonomy lock trong STUDY_RULES của study)
...
Study có thể thêm audience file riêng nếu có stakeholder mới (document trong STUDY_RULES của study).
...
1. Fill data (CSVs) — `participants.csv` + `quotes.csv` (schema lock trong STUDY_RULES)
...
| Label | Criteria |
|:--|:--|
| **Cao** | ≥5 nguồn độc lập (tính trên toàn mẫu), không ai phản bác trực tiếp → ra quyết định được |
| **Trung bình** | Có dấu hiệu rõ nhưng chưa đủ saturation, cần cohort tiếp xác nhận |
| **Tín hiệu sớm** | 1–2 nguồn, đáng flag nhưng cần research thêm |
| **Hypothesis** | Chưa có data trực tiếp, chỉ suy từ pattern khác |

Field value trong `quotes.csv`: `cao` / `trung_binh` / `som` / `hypothesis`.

Pattern-level tier (pretour-research-2026): `shared/viz_helpers.py` → `PATTERN_META` + `CONFIDENCE_TIERS`. Caveats từng finding: [studies/pretour-research-2026/STUDY_RULES.md](studies/pretour-research-2026/STUDY_RULES.md) mục 5.
...
Mẫu số (denominator) khi quote occurrence: **luôn ghi rõ** (vd "10/14" hoặc "5/8 chưa-từng"). KHÔNG hardcode — compute từ CSV (Rule #1 trong STUDY_RULES).
...
2. Tạo `studies/<new-study>/STUDY_RULES.md` lock rule chuyên biệt: sample size N, taxonomy IDs, terminology bất khả xâm phạm, confidence threshold cụ thể
3. Tạo `data/participants.csv` + `data/quotes.csv` với schema lock trong STUDY_RULES (có thể extend nếu methodology khác)
...
Nếu methodology mới (focus group, ethnography...) cần extra fields trong CSV, document trong STUDY_RULES mục Schema.
